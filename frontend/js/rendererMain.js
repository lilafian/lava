...
function isUrlOrSearchTerm(input) {
  ...
  const urlRegex =
    /^((?!-)[A-Za-z0-9-]{1,63}(?<!-)\.)+[A-Za-z]{2,6}(\/[A-Za-z0-9-._~:/?#[\]@!$&'()*+,;=%]*)?$/;
...
urlBar.addEventListener("keydown", async function (e) {
  if (e.code === "Enter") {
    if (isUrlOrSearchTerm(urlBar.value) === "httpUrl") {
      ...
      const settings = await getSettings();
      const searchEngineUrl = settings.searchEngine;
      console.log(searchEngineUrl);
      const searchUrl = searchEngineUrl.replaceAll("%s", urlBar.value);
      document.querySelector(".webcontent").loadURL(searchUrl);
    }
  }
});
